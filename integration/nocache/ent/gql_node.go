...
var nocacheImplementors = []string{"NoCache", "Node"}
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	switch table {
	case "NoCache":
		query := c.NoCache.Query().
			Where(nocache.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, nocacheImplementors...); err != nil {
				return nil, err
			}
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case "NoCache":
		query := c.NoCache.Query().
			Where(nocache.IDIn(ids...))
		query, err := query.CollectFields(ctx, nocacheImplementors...)
		if err != nil {
			return nil, err
		}
